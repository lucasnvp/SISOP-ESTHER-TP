...
uint32_t almacenarBytesPagina(uint32_t PID, uint32_t pagina, uint32_t offset, uint32_t size, void * buffer) { //TODO: Si ya esta en cache, actualizar contenido
//uint32_t cantMarcosOcupaMemoriaAdm = ((sizeof(uint32_t*) * 3 * config.MARCOS) + config.MARCO_SIZE - 1) / config.MARCO_SIZE;
	...
	memcpy(bloque_Memoria + tablaEPI.matriz[frame][N_FRAME] * config.MARCO_SIZE	+ offset, buffer, size);
	encontrePagina = 1;
...
//uint32_t cantMarcosOcupaMemoriaAdm = ((sizeof(uint32_t*) * 3 * config.MARCOS) + config.MARCO_SIZE - 1) / config.MARCO_SIZE;
	...
	if (!encontreEnCache) {
		...
		aux = malloc(size);
		memcpy(aux,	bloque_Memoria + tablaEPI.matriz[frame][N_FRAME] * config.MARCO_SIZE + offset, size);
		encontrePagina = 1;
		actualizoCache(PID, pagina, tablaEPI.matriz[frame][N_FRAME]);
	}
...
//uint32_t encontreEnCache = 0;
	...
	if (entradasCachePorPID == config.CACHE_X_PROC) {
		for (i = 0; i < config.ENTRADAS_CACHE; i++) {
			if (adminCache[i].memoriaCache.PID == PID) {
...
void connection_handler(uint32_t socket, uint32_t command) {
	...
	case 4: {
		...
		if (largoCadena > 0) {
			...
			for (i = 0; i < cantPaginas; i++)
				almacenarBytesPagina(PID, i, 0, config.MARCO_SIZE, cadena->dataString + i * config.MARCO_SIZE);
		}
		free(cadena);
	...
	case 6: {
		...
		char* instruccion = solicitarBytesPagina(nuevoPedido->id, nuevoPedido->pagina, nuevoPedido->offset, nuevoPedido->size);
